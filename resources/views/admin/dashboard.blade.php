        ...
        <section class="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-12">
            <div class="lg:col-span-3">
                <header class="flex items-center justify-between mb-8">
                                                                                                                        <h3 class="text-sm font-bold text-slate-900" style="font-family: 'Open Sans', Helvetica, Arial, sans-serif !important;">Available Quizzes</h3>
                                                            <p class="text-xs font-medium text-indigo-600">Quizzes waiting to be completed</p>
                </header>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    @forelse($studentQuizzes as $quiz)
                    <a href="{{ route('students.quizzes.take', $quiz->id) }}" class="group bg-white rounded-3xl border border-slate-100 p-7 shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 no-underline block">
                        <div class="flex flex-col h-full">
                            <!-- Title Tier -->
                            <div class="mb-5">
                                <h4 class="text-xl font-extrabold text-slate-900 tracking-tight mb-1 group-hover:text-indigo-600 transition-colors">{{ $quiz->title }}</h4>
                                <div class="flex items-center gap-3">
                            ...
                            <div class="mt-auto flex items-center justify-between pt-6 border-t border-slate-50">
                                <div class="flex items-center gap-5">
                                    <div class="flex items-center gap-2 text-slate-400">
                                        ...
                                        <span class="text-[11px] font-black text-slate-600 tracking-tight">{{ $quiz->questions_count ?? 0 }} Items</span>
                                    </div>
                                    <div class="flex items-center gap-2 text-slate-400">
                                ...
                                <div class="text-[10px] font-black text-indigo-600 uppercase tracking-[0.2em] flex items-center gap-2 group-hover:gap-3 transition-all">
                    ...
                    <div class="col-span-full py-32 bg-slate-50/50 rounded-[40px] border border-dashed border-slate-200 text-center">
                        <i class="fas fa-search text-3xl text-slate-200 mb-6"></i>
                                                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">No available quizzes detected.</p>
                    </div>
                    @endforelse
                </div>
            </div>
